...
exports.Login = async (req, res, next) => {
  try {
    ...
    const token = jwt.sign({ id: Login._id }, "smartbid-secret");
...
exports.Home = async (req, res) => {
  try {
    ...
    let EarningsAndSales = {
      income: productCost[0].totalCost * 0.05,
      productsSold: productQuantity,
    ...
    const ProductsByCategory = await Product.find({ sold: "true" }).select(
      "-image"
    );
...
exports.AllBanned = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: 200, message: "success", data: bannedUserData });
  } catch (err) {
    console.log(err);
    res.status(404).json({ status: 404, message: "fail", data: err.message });
  }
...
exports.AllProducts = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: 200, message: "success", data: productData });
  } catch (err) {
    console.log(err);
    res.status(404).json({ status: 404, message: "fail", data: err.message });
  }
...
exports.protect = async (req, res, next) => {
  try {
    ...
    const decoded = jwt.verify(token, "smartbid-secret");
    ...
    console.error("Error in protect middleware:", err); // Log error
    res.status(401).json({
      status: "fail",
...
exports.ViewAdminProfile = async (req, res) => {
  try {
    ...
    // Find the admin by ID and exclude the password from the response
    const adminProfile = await Admin.findById(adminId).select("-password");

    // Check if the admin profile was found
    if (!adminProfile) {
      return res
        .status(404)
    ...
    // Log the admin profile being sent
    console.log("Sending Admin Profile:", adminProfile);

    // Respond with the admin profile
    res.status(200).json({ status: "success", data: adminProfile });
  } catch (err) {
    console.error("Error fetching Admin Profile:", err); // Log error
