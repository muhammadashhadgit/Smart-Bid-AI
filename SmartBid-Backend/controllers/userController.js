...
exports.SendEmailVerification = async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: UserPinUpdated._id }, "smartbid-secret");
  ...
  try {
    ...
    if (UserInfo.pin !== req.body.pin) {
      ...
      const querySecond = Notification.create({
        ...
        notification: {
          content:
            "Welcome to SMARTBID! Charge your wallet and start bidding now!",
          date: req.body.dateApi,
          time: req.body.timeApi,
        },
      });
      const NotificationExtract = await querySecond;
      // Update email verified status
      UserInfo.emailVerified = true; // Assuming you have an emailVerified field
      const updateUserResponse = await UserInfo.save();

      const token = jwt.sign({ id: UserInfo._id }, "smartbid-secret");
      ...
      res.status(200).json({
        ...
        token: token,
        data: UserInfo,
        notification: NotificationExtract,
      });
    }
  } catch (err) {
    console.log(err);
...
exports.Login = async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: FindUser._id }, "smartbid-secret");
    ...
    const querySecond = User.updateOne(
      { _id: FindUser._id },
      { active: true },
      { new: true, runValidators: true }
    );
    ...
    res
      .status(200)
      .json({ status: 200, message: "success", token: token, data: FindUser });
  } catch (err) {
    console.log(err);
    res.status(404).json({ status: 404, message: "fail", data: err.message });
  }
...
exports.ViewProfile = async (req, res) => {
  try {
    console.log("user controller view profile params: ", req.body);
    ...
    res
      .status(200)
      .json({ status: 200, message: "success", data: viewProfile });
  } catch (err) {
    console.log(err);
    res.status(404).json({ status: 404, message: "fail", data: err.message });
  }
...
exports.EditProfile = async (req, res) => {
  try {
    ...
    if (req.body.password) {
      update.password = pbkdf2.pbkdf2Sync(
        req.body.password,
        "smartbid-secret",
        1,
        32,
...
const getHighestProductId = async () => {
  try {
    const highestProduct = await Product.findOne().sort({ _id: -1 });
    return highestProduct ? highestProduct._id : 0;
  } catch (error) {
    console.error('Error finding the highest product ID:', error);
    throw error;
  }
};
...
exports.AddProduct = async (req, res) => {
  try {
    ...
    console.log("The highest Product Id is : ", await getHighestProductId());


    // Query to add the product with its details to the Database
    const query = Product.create({
      _id: await getHighestProductId() + 1,
      name: req.body.name,
    ...
    res
      .status(201)
      .json({ status: 201, message: "success", data: productAdded });
  } catch (err) {
    console.log(err);
    res.status(404).json({ status: 404, message: "fail", data: err.message });
  }
...
exports.BidOnProduct = async (req, res) => {
  try {
    ...
    if (loopCheck == true) {
      walletAfterBid =
        biddingArray[matchedValueCounter].bidCost +
        UserFound.wallet -
        req.body.bidCost;
    ...
    if (ProductFound.cost > req.body.bidCost) {
      throw new Error(
        "The bid amount should be greater than the original cost of the product"
      );
    }
    ...
    const query = Product.updateOne(filter, update, {
      new: true,
      runValidators: true,
    });
    ...
    const queryWallet = User.updateOne(
      { _id: req.body.userID },
      { wallet: walletAfterBid },
      { new: true, runValidators: true }
    );
    ...
    res
      .status(200)
      .json({ status: 200, message: "success", data: bidOnProduct });
  } catch (err) {
    console.log(err);
    res.status(404).json({ status: 404, message: "fail", data: err.message });
  }
...
exports.ViewCurrentBidProducts = async (req, res) => {
  try {
    ...
    const query = Product.find({ sold: "false" }).elemMatch("bid", {
      userID: req.body.userID,
    });
...
exports.ViewMyProducts = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: 200, message: "success", data: viewProducts });
  } catch (err) {
    console.log(err);
    res.status(404).json({ status: 404, message: "fail", data: err.message });
  }
...
exports.SubmitReviewToSeller = async (req, res) => {
  try {
    ...
    const update = {
      $push: {
        reviewAsSeller: finalObject,
        ratingArrayAsSeller: req.body.rating,
      },
    };
    ...
    const querySecond = User.findOneAndUpdate(
      filter,
      { ratingAsSeller: rating },
      { new: true, runValidators: true }
    );
    const updateRating = await querySecond;

    // Generate a notification of rating to the Seller
    const queryThird = Notification.findOneAndUpdate(
      filterNotification,
...
exports.SubmitReviewToBidder = async (req, res) => {
  try {
    ...
    const update = {
      $push: {
        reviewAsBidder: finalObject,
        ratingArrayAsBidder: req.body.rating,
      },
    };
...
exports.ChargeWallet = async (req, res) => {
  try {
    ...
    const querySecond = User.findOneAndUpdate(filter, update, {
      new: true,
      runValidators: true,
    });
    ...
    res
      .status(200)
      .json({ status: 200, message: "success", data: UpdatedAmount });
  } catch (err) {
    console.log(err);
    res.status(404).json({ status: 404, message: "fail", data: err.message });
  }
...
exports.ChatList = async (req, res) => {
  try {
    ...
    const query = Product.find({
      sold: "true",
      $or: [{ userID: req.body.userID }, { newOwner: req.body.userID }],
    ...
    const querySecond = User.find({ _id: { $in: arr } }).select(
      "-password -phoneNumber -address -wallet"
    );
...
exports.UserChat = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: 200, message: "success", data: MessageList });
  } catch (err) {
    console.log(err);
    res.status(404).json({ status: 404, message: "fail", data: err.message });
  }
...
exports.DeleteAccount = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: 200, message: "success", data: "Account is Deleted" });
  } catch (err) {
    console.log(err);
    res.status(404).json({ status: 404, message: "fail", data: err.message });
  }
...
exports.UpdateHistory = async (req, res) => {
  try {
    ...
    console.log("update history : " + req.body.id)
    const query = userHistory.find({ _id: req.body.id });
    const historyObject = await query;

    console.log(historyObject)

    let int_id_product = parseInt(req.body.productID);

    if (historyObject.length == 0) {
      console.log("Creating History : ")
      userHistory.create({ _id: req.body.id, history: [int_id_product] })
    }
    else {
      console.log("Appending to History")
      let hist = historyObject[0].history; 
      console.log(hist)
      if (!hist.includes(int_id_product)) {
        hist.push(int_id_product);
        const update = { history: hist};
        const options = { new: true };

        UserHistory.findByIdAndUpdate(req.body.id, update, options)
    ...
    res
      .status(200)
      .json({ status: 200, message: "success", data: "History is Updated" });
  }
  catch (err) {
    console.log(err);
    res.status(404).json({ status: 404, message: "fail", data: err.message });
