...
exports.SearchProduct = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: 200, message: "success", data: searchProduct });
  } catch (err) {
    console.log(err);
...
exports.ViewSingleProduct = async (req, res, next) => {
  try {
    ...
    console.log(
      "Recommendations Request single:",
  ...
  try {
    ...
    res
      ...
      .json({
        status: 200,
        message: "success",
        data: req.body.timeRemaining,
        highestBidder: maxBid,
      });
  } catch (err) {
    console.log(err);
    res.status(404).json({ status: 404, message: "fail", data: err.message });
...
exports.ViewRecommendations = async (req, res) => {
  try {
    const user = await userHistory.findById(req.query.id);
    ...
    let ViewProducts = await query;

    let finalData = ViewProducts.filter((product) => {
      return response.data.recommendations.includes(product._id);
    });
