describe("Add Balance Functionality", () => {
  it("Balance Added Successfully", () => {
    ...
    cy.url().should("eq", "http://localhost:8000/wallet");
    cy.wait(3000);
    cy.get("#current-balance")
      ...
      .then((originalValue) => {
        ...
        cy.url().should("eq", "http://localhost:8000/payment");
        ...
        cy.get("#current-balance")
          ...
          .then((newValue) => {
            ...
            expect(parseInt(newValue.substring(1))).to.equal(
              parseInt(originalValue.substring(1)) + 2000
            );
          });
      });
  });
